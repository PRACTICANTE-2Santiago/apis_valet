...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if(isset($_GET['placas'])){
        $placas = $conn->real_escape_string($_GET['placas']);
        $sql = $conn->query("SELECT * FROM automovil where placas = '$placas' ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = $conn->query("INSERT INTO automovil (id,
     id_comercios,
     id_chofer, 
     placas, 
     descripcion, 
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    if (isset($_GET['id'])) {
        $id = $conn->real_escape_string($_GET['id']);
        $data = json_decode(file_get_contents("php://input"));
        $sql = $conn->query("UPDATE automovil SET estatus = '" . $data->estatus . "'
         WHERE id = '$id'");
        if ($sql) {
